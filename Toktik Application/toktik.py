...
oldestDateTime = "11/12/2021 00:00:00"

# widgets

st.title('TokTik')
st.markdown("Welcome to TokTik, your fake TikTok app!")
...
def sendFile():
    ...
    fileType = prefix + "/" + ext
    backend.createPost("account", fileName, fileType)  # remove account field
...
def getLatestFeeds():
    ...
    if not mediaList:
        if not feed:
            ...
            latest = feed.dateTime
    else:
        latest = mediaList[-1].dateTime
